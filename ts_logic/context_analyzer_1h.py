...
if __name__ == '__main__':
    print("Тестирование context_analyzer_1h.py (логика линий тренда)...")
    ...
    for line in trend_lines_up:
        print(f"  Start: {line['start_time']} @ {line['start_price']}, End: {line['end_time']} @ {line['end_price']}, Color: {line['color']}, Style: {line.get('lineStyle', 'Solid')}")

    # Пример данных структуры (нисходящий тренд)
    structure_down = [
        {'time': pd.Timestamp('2023-01-02 10:00', tz='UTC'), 'price': 120, 'type': 'H'},
        {'time': pd.Timestamp('2023-01-02 11:00', tz='UTC'), 'price': 110, 'type': 'L'},
        {'time': pd.Timestamp('2023-01-02 12:00', tz='UTC'), 'price': 115, 'type': 'LH'},
        {'time': pd.Timestamp('2023-01-02 13:00', tz='UTC'), 'price': 105, 'type': 'LL'},
        {'time': pd.Timestamp('2023-01-02 14:00', tz='UTC'), 'price': 108, 'type': 'LH'},
        {'time': pd.Timestamp('2023-01-02 15:00', tz='UTC'), 'price': 98, 'type': 'LL'}
    ]
    context_down = determine_overall_market_context(structure_down)
    print(f"\nПример 2: Нисходящий тренд. Контекст: {context_down}")
    trend_lines_down = determine_trend_lines(structure_down, context_down)
    print("Определенные линии тренда:")
    for line in trend_lines_down:
        print(f"  Start: {line['start_time']} @ {line['start_price']}, End: {line['end_time']} @ {line['end_price']}, Color: {line['color']}, Style: {line.get('lineStyle', 'Solid')}")
    ...
    structure_range = [
        ...
        {'time': pd.Timestamp('2023-01-03 13:00', tz='UTC'), 'price': 107, 'type': 'HH'} # Пробитие, но может быть в широком диапазоне
    ]
    context_range = determine_overall_market_context(structure_range)
    print(f"\nПример 3: Консолидация/Диапазон. Контекст: {context_range}")
